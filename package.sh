...
# Delete old zip
[ -e kiosk-poc.zip ] && rm kiosk-poc.zip
...
mkdir -p metadata;
echo -n "firmware::info:"$(jq --compact-output ". + {built: now | todateiso8601} + {builder: \""$(whoami)"\"}" kiosk-poc.json) | tr -d '\n' | tr -d '\r'  >metadata/info.bin
printf '\0' >>metadata/info.bin

# Build the ZIP
echo "Building kiosk-poc.zip"
cat kiosk-poc.json
echo ""
